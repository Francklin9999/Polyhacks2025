...
def calculate_planetary_parameters(D, M, T, P, W, B, θ, R, G, X):
    """
    Calculates planetary parameters and the probability of life on a scale of 0-100.
    """
    ...
    # Habitability factors with improved scaling
    f_T = min(1, max(0, math.exp(-abs(T - T_E) / T_E)))  # Temperature factor
    f_P = math.exp(-((math.log(P) - math.log(P_E))**2))  # Atmospheric balance (centered at Earth)
    f_W = W  # Water presence (1 if water exists, 0 otherwise)
    f_B = B / (1 + abs(B - B_E))  # Magnetic field protection
    f_G = G  # Geological activity
    f_X = 1 / (1 + (X / X_E))  # Radiation survival factor (better scaling)

    # Compute probability of life (scale 0-100)
    P_L = 100 * (f_T * f_P * f_W * f_B * f_G * f_X)
